...
An existing configured JSONL root must be a readable directory when the plugin loads. An absent root remains valid and is created on first materialization. Physical writes since the [cross-process writer lock](../architecture/2026-08-15-jsonl-cross-process-writer-lock.md) enforce the one-live-writer storage topology at commit time: a second backend instance or process mutating the same session fails instead of corrupting the log.
...
**Coordinate multiple live writers.** A dedicated coordination service or process-global registry would define a new deployment topology rather than repair identity validation; no-overwrite hard-link publication still arbitrates an initial same-id creation race. A cross-process writer lock for physical writes later shipped for log integrity — see the [cross-process writer lock note](../architecture/2026-08-15-jsonl-cross-process-writer-lock.md) — without changing this note's identity-validation scope.

## Consequences

Mismatched, misplaced, and duplicate JSONL logs fail before repair or coordinator state mutation. Lookup remains proportional to the number of project directories; one-live-writer ownership is enforced by the [cross-process writer lock](../architecture/2026-08-15-jsonl-cross-process-writer-lock.md). Coordinator and JSONL tests pin rejection before repair, unchanged bytes for both affected logs, path validation during listing, duplicate-id rejection, normalized-project collisions and case aliases, and load-time root validation.
