...
如果配置的 JSONL 根目录已存在，插件加载时该路径必须是可读目录。根目录不存在仍然是有效配置，首次物化时会创建该目录。自[跨进程 writer lock](../architecture/2026-08-15-jsonl-cross-process-writer-lock.md) 起，物理写入在提交时强制单一存活 writer 的存储拓扑：变更同一会话的第二个后端实例或进程会失败，而不是损坏日志。
...
**协调多个活动写入方。** 专用协调服务或进程级全局注册表会定义新的部署拓扑，而不是修复身份验证；禁止覆盖的硬链接发布仍会裁决初始的同 id 创建竞态。针对物理写入的跨进程 writer lock 此后为日志完整性而落地——见[跨进程 writer lock Agent Note](../architecture/2026-08-15-jsonl-cross-process-writer-lock.md)——但不改变本 note 的身份校验范围。

## 后果

JSONL 日志的身份不匹配、位置错误和重复会在修复或协调器状态变更前失败。查找开销仍与项目目录数量成正比；单一存活 writer 的所有权由[跨进程 writer lock](../architecture/2026-08-15-jsonl-cross-process-writer-lock.md) 强制。协调器和 JSONL 测试固定了修复前拒绝、两个受影响日志的字节均保持不变、列出时的路径验证、重复 id 拒绝、项目路径规范化冲突与大小写别名，以及加载时的根目录验证。
