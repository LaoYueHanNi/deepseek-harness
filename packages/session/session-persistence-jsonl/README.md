...
- **Append-only.** Flushed events are never rewritten. Subsequent raw batches append lines; compressed batches append one frame. Both paths `fsync`, and a caught write or sync failure rolls the file back to its prior byte length. Every physical mutation — append, materialization, and repair — holds a cross-process OS advisory lock on a `session.jsonl[.zstd].lock` sibling (`flock` on POSIX, `LockFileEx` on Windows) and verifies the durable tail first: the batch's first `seq` must continue the seq the stored log actually ends at, checked against a stat-revision-keyed tail cache. A stale in-memory cursor (another process or backend instance appended first) fails with a tail-divergence error naming both seqs and leaves the file untouched; a lock held by another process fails immediately. The kernel releases the lock when its owner exits, so a crashed writer never strands one. Crash-repair markers carry the log revision they were read at, and a repair whose revision no longer matches aborts instead of truncating events a concurrent writer committed.
...
- **One live writer per session, enforced at commit time** — a second backend instance or process appending from a stale in-memory cursor fails with a tail-divergence error instead of writing a seq fork, and fails immediately while another process holds the writer lock; a writer that reloads between batches continues the same log legitimately. Lock files accumulate beside logs until external cleanup; discovery reads only the fixed transcript names. Design rationale: [the cross-process writer lock note](../../../.agents/notes/implemented/architecture/2026-08-15-jsonl-cross-process-writer-lock.md).
